  ...
  class ServiceCallflowOutputFull
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ServiceCallflowOutputFull` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ServiceCallflowOutputFull`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'featurecode')
        self.featurecode = attributes[:'featurecode']
      end
    ...
    def list_invalid_properties
      ...
      if @patterns.nil?
        invalid_properties.push('invalid value for "patterns", patterns cannot be nil.')
      end
    ...
    def flow=(flow)
      if flow.nil?
        fail ArgumentError, 'flow cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] patterns Value to be assigned
    def patterns=(patterns)
      if patterns.nil?
        fail ArgumentError, 'patterns cannot be nil'
      end
    ...
    def hash
      [featurecode, flow, id, name, numbers, patterns].hash
    end
