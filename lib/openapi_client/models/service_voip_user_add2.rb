  ...
  class ServiceVOIPUserAdd2
    ...
    def self.openapi_types
      {
        ...
        :'caller_id' => :'ModelsUserOutputFullCallerid',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ServiceVOIPUserAdd2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ServiceVOIPUserAdd2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @first_name.to_s.length > 128
      return false if @last_name.nil?
      return false if @last_name.to_s.length > 128
      true
    end
    ...
    def hash
      [call_forward, call_recording, caller_id, do_not_disturb, email, enabled, first_name, last_name, music_on_hold, presence_id, timezone, verified].hash
    end
